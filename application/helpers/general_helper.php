...
/**
 * Calculate student's fee payment percentage
 * 
 * @param int $enroll_id The enrollment ID of the student
 * @param int $session_id The academic session ID
 * @return float The percentage of fees paid (0-100)
 */
function get_student_fee_payment_percentage($enroll_id, $session_id)
{
    $CI = &get_instance();

    // Get total fees allocated
    $sql = "SELECT SUM(`fee_groups_details`.`amount` + `fee_allocation`.`prev_due`) as `total_amount` 
            FROM `fee_allocation` 
            LEFT JOIN `fee_groups_details` ON `fee_groups_details`.`fee_groups_id` = `fee_allocation`.`group_id` 
            WHERE `fee_allocation`.`student_id` = " . $CI->db->escape($enroll_id) . " 
            AND `fee_allocation`.`session_id` = " . $CI->db->escape($session_id);
    $total = $CI->db->query($sql)->row_array();
    $total_amount = isset($total['total_amount']) ? floatval($total['total_amount']) : 0;

    // Get total transport fees if transport module is enabled
    if (moduleIsEnabled('transport')) {
        $CI->db->select("IFNULL(SUM(transport_stoppage_point.route_fare), 0) as amount");
        $CI->db->from('transport_fee_details');
        $CI->db->join('transport_stoppage_point', 'transport_stoppage_point.id = transport_fee_details.stoppage_point_id', 'inner');
        $CI->db->where('transport_fee_details.enroll_id', $enroll_id);
        $trans_amount = $CI->db->get()->row()->amount;
        $total_amount += floatval($trans_amount);
    }

    // Get total paid amount
    $CI->db->select("IFNULL(SUM(fee_payment_history.amount), 0) as paid_amount, IFNULL(SUM(fee_payment_history.discount), 0) as discount_amount");
    $CI->db->from("fee_payment_history");
    $CI->db->join("fee_allocation", "fee_allocation.id = fee_payment_history.allocation_id", "left");
    $CI->db->join("transport_fee_details", "transport_fee_details.id = fee_payment_history.transport_fee_details_id", "left");
    $CI->db->where("fee_allocation.student_id", $enroll_id);
    $CI->db->where("fee_allocation.session_id", $session_id);
    $CI->db->or_group_start();
    $CI->db->where("transport_fee_details.enroll_id", $enroll_id);
    $CI->db->group_end();
    $paid = $CI->db->get()->row_array();

    $total_paid = isset($paid['paid_amount']) ? floatval($paid['paid_amount']) : 0;
    $total_discount = isset($paid['discount_amount']) ? floatval($paid['discount_amount']) : 0;
    ...
    if ($total_amount <= 0) {
        return array(
            ...
            'paid_amount' => 0,
            'remaining_amount' => 0
        ); // No fees allocated means 100% paid
    }

    $payment_percentage = (($total_paid + $total_discount) / $total_amount) * 100;
    $remaining_amount = $total_amount - ($total_paid + $total_discount);

    return array(
        'percentage' => round($payment_percentage, 2),
        'total_amount' => $total_amount,
        'paid_amount' => $total_paid + $total_discount,
        'remaining_amount' => max(0, $remaining_amount)
    );
}

/**
 * Check if student can view exam results based on fee payment threshold
 * 
 * @param int $enroll_id The enrollment ID of the student
 * @param int $exam_id The exam ID
 * @param int $session_id The academic session ID
 * @return array Array with 'allowed' (bool) and 'percentage' (float) and 'threshold' (float)
 */
function can_view_exam_results($enroll_id, $exam_id, $session_id)
{
    $CI = &get_instance();

    // Get exam fee threshold
    $exam = $CI->db->select('fee_threshold_percentage')->where('id', $exam_id)->get('exam')->row();

    if (!$exam) {
        return array('allowed' => false, 'percentage' => 0, 'threshold' => 0, 'message' => 'Exam not found');
    }

    $threshold = floatval($exam->fee_threshold_percentage);

    // If threshold is 0, no restriction
    if ($threshold <= 0) {
        return array('allowed' => true, 'percentage' => 100, 'threshold' => 0, 'message' => '');
    }

    // Get student's payment percentage and amounts
    $payment_data = get_student_fee_payment_percentage($enroll_id, $session_id);
    $payment_percentage = $payment_data['percentage'];
    $total_amount = $payment_data['total_amount'];
    $paid_amount = $payment_data['paid_amount'];
    $remaining_amount = $payment_data['remaining_amount'];

    $allowed = ($payment_percentage >= $threshold);
    $message = '';

    if (!$allowed) {
        $remaining_percent = $threshold - $payment_percentage;
        $required_amount = ($threshold / 100) * $total_amount;
        $still_needed = $required_amount - $paid_amount;

        $currency = get_global_setting('currency');
        $message = sprintf(
            'You must pay at least %s%% (%s%s) of your school fees to view exam results. Current payment: %s%% (%s%s). Amount needed: %s%s',
            number_format($threshold, 2),
            $currency,
            number_format($required_amount, 2),
            number_format($payment_percentage, 2),
            $currency,
            number_format($paid_amount, 2),
            $currency,
            number_format($still_needed, 2)
        );
    }
